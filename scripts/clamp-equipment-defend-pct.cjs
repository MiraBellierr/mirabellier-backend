...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (arg === "--help" || arg === "-h") {
      console.log(
        [
          "Usage: node scripts/clamp-equipment-defend-pct.cjs [flags]",
          "",
          "Clamps all defendPct sub-stats on equipment pieces down to " + DEFEND_PCT_MAX + ".",
          "",
          "Flags:",
...
function clampDefendPct(db, { apply = false, backupFile = "" } = {}) {
...
function main() {
  ...
  let result;
  try {
    result = clampDefendPct(db, options);
  } finally {
    db.close();
  }
  ...
  if (result.affected.length === 0) {
    console.log("No defendPct values exceed", DEFEND_PCT_MAX + ".");
    return;
  }

  console.log(
    `Found ${result.affected.length} defendPct sub-stats over ${DEFEND_PCT_MAX} across ${result.dirtyCount} pieces:\n`,
  );

  for (const entry of result.affected) {
    console.log(
      `  ${entry.id} (${entry.slot}): defendPct ${entry.before} → ${entry.after}`,
    );
  }
  ...
  if (!options.apply) {
    ...
    console.log(
      `\nApplied: ${result.affected.length} defendPct values clamped across ${result.dirtyCount} pieces.`,
    );
  }
}
...
module.exports = { clampDefendPct, DEFEND_PCT_MAX };
