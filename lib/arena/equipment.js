...
const DEFAULT_FODDER_REFUND = 500;

// Refund for scrapping a piece is derived entirely server-side from its slot's
// shop price. The client no longer has any say in the payout.
function getFodderRefund(slot) {
  const shopEntry = ROLLABLE_EQUIPMENT.find((entry) => entry.slot === slot);
  const purchasePrice = toPositiveInt(shopEntry?.price, 0);
  return purchasePrice > 0 ? Math.floor(purchasePrice / 2) : DEFAULT_FODDER_REFUND;
}
...
function toPublicEquipmentPiece(row, includeEquipped = false) {
  ...
  const piece = {
    ...
    fodderRefund: getFodderRefund(row.slot),
...
function fodderEquipmentPiece(db, userId, pieceId) {
  const piece = db.prepare(
    "SELECT slot, equipped, locked FROM arena_equipment_pieces WHERE id = ? AND userId = ?",
  ).get(pieceId, userId);
  ...
  const FODDER_PRICE = getFodderRefund(piece.slot);
  const now = nowIso();
  const tx = db.transaction(() => {
