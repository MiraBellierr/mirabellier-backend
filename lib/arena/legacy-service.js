...
function chooseEloOpponent(db, userId, randomFn = Math.random) {
  ...
  const lastOpponent = db
    .prepare(
      `SELECT opponentUserId FROM arena_fights WHERE userId = ? AND opponentUserId NOT LIKE 'npc:%' ORDER BY createdAt DESC LIMIT 1`,
    )
    .get(userId);

  const lastOpponentId = lastOpponent?.opponentUserId || null;
