...
function applyFightEffectUsage(effects, effectUsage) {
  ...
  if (effectUsage.usedHigherRarityBonus && next.higherRarityDamageBonusPctCharges > 0) {
    console.log("[HIGHER_RARITY_BONUS] DECREMENTING from", next.higherRarityDamageBonusPctCharges, "to", next.higherRarityDamageBonusPctCharges - 1);
...
async function simulateFight(db, input) {
  ...
  console.log("[HIGHER_RARITY_BONUS] player charges:", playerEffects.higherRarityDamageBonusPctCharges, "player rarity:", playerRarity, "(rank", rarityRank(playerRarity), ")", "opponent rarity:", opponentRarity, "(rank", rarityRank(opponentRarity), ")");
  ...
  console.log("[HIGHER_RARITY_BONUS] hasHigherRarityBonus:", hasHigherRarityBonus, "pct:", playerEffects.higherRarityDamageBonusPct);
...
async function runFight(db, userId) {
  ...
  const tx = db.transaction(() => {
    ...
    if (simulation.playerWon) {
      ...
      coinDelta = 3 + Math.floor(current.level / 3);
    }
...
function finalizePlaybackFightRewards(db, userId) {
  ...
  if (simulation.rewards && typeof simulation.rewards === "object") {
    console.log("[HIGHER_RARITY_BONUS] finalize - EARLY RETURN, rewards already exist");
    return { rewards: simulation.rewards };
  }
  console.log("[HIGHER_RARITY_BONUS] finalize - PROCEEDING to apply effects");
  ...
  const tx = db.transaction(() => {
    ...
    console.log("[HIGHER_RARITY_BONUS] finalize - effectUsage:", JSON.stringify(effectUsage));
    console.log("[HIGHER_RARITY_BONUS] finalize - charges before:", effects.higherRarityDamageBonusPctCharges);
    const nextEffects = applyFightEffectUsage(effects, effectUsage);
    console.log("[HIGHER_RARITY_BONUS] finalize - charges after:", nextEffects.higherRarityDamageBonusPctCharges);
