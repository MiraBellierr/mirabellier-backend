...
function chooseEloOpponent(db, userId, randomFn = Math.random) {
  ...
  const queryAdjacent = (operator, order) =>
    db
      .prepare(
        `SELECT p.* FROM arena_profiles p
         JOIN users u ON u.id = p.userId
         WHERE p.userId <> ?
           AND p.selectedCardJson IS NOT NULL
           AND p.eloRating ${operator} ?
           AND (p.lastOpponentDate != date('now') OR p.dailyOpponentCount < ?)
         ORDER BY p.eloRating ${order}
         LIMIT 1`,
      )
      .get(userId, player.eloRating, MAX_DAILY_OPPONENT_COUNT);

  const above = mapArenaProfileRow(queryAdjacent(">", "ASC"));
  const below = mapArenaProfileRow(queryAdjacent("<", "DESC"));

  const candidates = [above, below].filter(Boolean);
