...
function calculateAttackOutcome(input) {
  ...
  const defenseRoll =
    ...
    (Number(RARITY_CONFIG[defenderRarity]?.powerBonus || 0)) * 0.2 +
...
function chooseEloOpponent(db, userId, randomFn = Math.random) {
  ...
  const candidates = db
    .prepare(
      `SELECT p.* FROM arena_profiles p JOIN users u ON u.id = p.userId WHERE p.userId <> ? AND p.selectedCardJson IS NOT NULL AND (p.lastOpponentDate != date('now') OR p.dailyOpponentCount < ?) ORDER BY ABS(COALESCE(p.eloRating, ?) - ?) ASC, COALESCE(p.eloMatches, 0) DESC LIMIT ?`,
    )
    .all(userId, MAX_DAILY_OPPONENT_COUNT, ELO_DEFAULT_RATING, player.eloRating, ELO_MATCHMAKING_CANDIDATE_LIMIT * 2)
  ...
  db.prepare(
    `UPDATE arena_profiles
     SET dailyOpponentCount = CASE
       WHEN lastOpponentDate = ? THEN dailyOpponentCount + 1
       ELSE 1
     END,
     lastOpponentDate = ?
     WHERE userId = ?`,
