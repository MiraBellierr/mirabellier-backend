...
const { rollEquipmentPiece, insertEquipmentPiece, equipEquipmentPiece, computeEquipmentStats, getFodderRefund } = require("./equipment");
...
function buildShopCatalog(profile, inventoryMap) {
  ...
  SHOP_ITEMS.forEach((item) => {
    ...
    if (item.acquisition === "craft" && recipe && unlocked) {
      ...
      canCraft = hasCoins && underInventoryCap;
    }
...
function getArenaShopPayload(db, userId) {
  ...
  const recipes = SHOP_RECIPES.map((recipe) => {
    ...
    return {
      ...
      unlocked,
      canCraft: unlocked && hasCoins && underInventoryCap,
    };
  });
...
function buyShopItem(db, userId, itemId) {
  ...
  const tx = db.transaction(() => {
    ...
    if (item.type === "gear") {
      ...
      rolledPieceData = { ...piece, fodderRefund: getFodderRefund(piece.slot) };
