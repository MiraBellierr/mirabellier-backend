    ...
    consumables: [
      {
        ...
        consumableEffect: { kind: "shield_fight_start", amount: 20, charges: 1 },
      },
      {
        id: "green_draft",
        name: "Green Draft",
        sprite: { sheet: "game.png", row: 8, col: 2, size: 32 },
        consumableEffect: { kind: "exp_boost", pct: 20, wins: 1 },
      },
      {
        id: "amber_draft",
        name: "Amber Draft",
        sprite: { sheet: "game.png", row: 8, col: 3, size: 32 },
        consumableEffect: { kind: "coin_boost", pct: 20, wins: 1 },
      },
    ],
    materials: [
      {
    ...
    consumables: [
      {
        ...
        consumableEffect: { kind: "evade_next_fight", pct: 15, fights: 1 },
      },
      {
        id: "viridian_elixir",
        name: "Viridian Elixir",
        sprite: { sheet: "game.png", row: 8, col: 6, size: 32 },
        consumableEffect: { kind: "upgrade_lowest_rarity", charges: 1 },
      },
      {
        id: "fuse_bomb",
        name: "Fuse Bomb",
        sprite: { sheet: "game.png", row: 9, col: 11, size: 32 },
        consumableEffect: { kind: "first_hit_true_damage", value: 25, charges: 1 },
      },
    ],
    materials: [
      {
    ...
    consumables: [
      {
        ...
        consumableEffect: { kind: "coin_boost", pct: 40, wins: 2 },
      },
      {
        id: "star_tonic",
        name: "Star Tonic",
        sprite: { sheet: "game.png", row: 8, col: 8, size: 32 },
        consumableEffect: { kind: "exp_boost", pct: 40, wins: 2 },
      },
      {
        id: "lantern_oil",
        name: "Lantern Oil",
        sprite: { sheet: "game.png", row: 9, col: 8, size: 32 },
        consumableEffect: { kind: "bonus_vs_higher_rarity", pct: 10, charges: 1 },
      },
    ],
    materials: [
      {
    ...
    consumables: [
      {
        ...
        consumableEffect: { kind: "reroll_keep_higher", charges: 1 },
      },
      {
        id: "oath_ribbon",
        name: "Oath Ribbon",
        sprite: { sheet: "game.png", row: 10, col: 10, size: 32 },
        consumableEffect: { kind: "streak_shield", charges: 2 },
      },
      {
        id: "treasure_cache",
        name: "Treasure Cache",
        sprite: { sheet: "game.png", row: 10, col: 11, size: 32 },
        consumableEffect: { kind: "coin_boost", pct: 80, wins: 3 },
      },
    ],
    materials: [
      {
    ...
    consumables: [
      {
        ...
        consumableEffect: { kind: "guarantee_ssr_plus", charges: 1 },
      },
      {
        id: "sacred_candles",
        name: "Sacred Candles",
        sprite: { sheet: "game.png", row: 9, col: 10, size: 32 },
        consumableEffect: { kind: "shield_fight_start", amount: 35, charges: 1 },
      },
      {
        id: "gate_key",
        name: "Gate Key",
        sprite: { sheet: "game.png", row: 10, col: 9, size: 32 },
        consumableEffect: { kind: "cooldown_bypass", charges: 1 },
      },
    ],
    materials: [
      {
...
const CONSUMABLE_CRAFT_COIN_FEES = [60, 250, 800, 2500, 9000, 30000];
...
function buildRecipes() {
  ...
  TIER_CONFIG.forEach((tierConfig, tierIndex) => {
    ...
    const [materialA, materialB, materialC] = tierConfig.materials.map((item) => item.id);

    const gearSpecs = [
      { inputs: [[materialA, 4], [materialB, 2], [materialC, 1]] },
      { inputs: [[materialA, 2], [materialB, 4], [materialC, 1]] },
      { inputs: [[materialA, 1], [materialB, 2], [materialC, 4]] },
      { inputs: [[materialA, 3], [materialB, 3], [materialC, 2]] },
    ];
    ...
    tierConfig.gear.forEach((gearItem, index) => {
      recipes.push({
        ...
        coinCost: GEAR_CRAFT_COIN_FEES[tierIndex],
        inputs: gearSpecs[index].inputs.map(([itemId, quantity]) => ({
          itemId,
          quantity,
        })),
      });
    ...
    const consumableSpecs = [
      { inputs: [[materialA, 2], [materialC, 1]] },
      { inputs: [[materialB, 2], [materialA, 1]] },
      { inputs: [[materialC, 2], [materialB, 1]] },
    ];
    ...
    tierConfig.consumables.forEach((consumableItem, index) => {
      recipes.push({
        ...
        coinCost: CONSUMABLE_CRAFT_COIN_FEES[tierIndex],
        inputs: consumableSpecs[index].inputs.map(([itemId, quantity]) => ({
          itemId,
          quantity,
        })),
      });
...
const SHOP_ITEMS = TIER_CONFIG.flatMap((tierConfig) => [
  ...
  ...tierConfig.materials.map((item, index) =>
    normalizeMaterialItem(tierConfig, item, index),
  ),
]).map((item) => ({
  ...item,
...
const MATERIAL_IDS_BY_TIER = TIER_CONFIG.map((entry) =>
  entry.materials.map((material) => material.id),
);
