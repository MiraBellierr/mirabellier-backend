...
// Consumables and their recipes are intentionally not level-locked — every tier
// is craftable from level 1 (see the "consumable recipes are not level locked"
// test). `unlockLevel: 1` below is the always-open gate that shop.js checks.
...
// Flat coin cost to craft any consumable of a given tier. Keyed by tier name so
// a new tier without a cost fails loudly at module load instead of silently
// crafting for free (coinCost: undefined -> toInt(..., 0)).
const CRAFT_COIN_COSTS = {
  Rookie: 200,
  Bronze: 800,
  Silver: 3200,
  Gold: 10000,
...
function buildRecipes() {
  ...
  TIER_CONFIG.forEach((tierConfig) => {
    const tierSlug = tierConfig.tier.toLowerCase();
    const coinCost = CRAFT_COIN_COSTS[tierConfig.tier];
    if (typeof coinCost !== "number") {
      throw new Error(
        `arena-constants: missing CRAFT_COIN_COSTS entry for tier "${tierConfig.tier}"`,
      );
    }
