...
function cardStatScore(card) {
  if (!card) return 0;
  return toInt(card.iv?.power, 0) + toInt(card.iv?.speed, 0) + toInt(card.iv?.guard, 0) + toInt(card.iv?.luck, 0);
}

function attackScore(card) {
  if (!card) return 0;
  return toInt(card.iv?.power, 0) + toInt(card.iv?.speed, 0) + toInt(card.iv?.luck, 0);
}
...
function playAiTurn(state, playerKey) {
  ...
  if (!playerState.drawnCardThisTurn && playerState.drawPile.length > 0 && state.turn >= 2) {
    const drawn = playerState.drawPile.pop();
    if (drawn) {
  ...
  if (!playerState.board.attacker) {
    ...
    if (bestIndex >= 0) {
      ...
      if (bestCard) {
        ...
        actions.push(`placed ${bestCard.title} as attacker`);
      }
    }
  }

  // 3. Attack if attacker has >= 2 elements
  if (playerState.board.attacker && (playerState.board.attacker.assignedElements?.length || 0) >= ELEMENTS_TO_ATTACK) {
  ...
  if (playerState.elementPool.length > 0) {
    ...
    if (targets.length > 0) {
      ...
      actions.push(`assigned ${poolEl} to ${targets[0].card.title}`);
    }
  }
  ...
  if (!playerState.placedCardThisTurn && playerState.hand.length > 0) {
    ...
    if (emptySupport >= 0) {
      ...
      if (bestCard) {
        ...
        playerState.placedCardThisTurn = true;
        actions.push(`placed ${bestCard.title} in support`);
      }
    }
  }
