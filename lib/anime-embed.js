...
function formatAnimeDetails(item) {
  return `Last update ${formatDateTime(item.updatedAt)} - ${formatSeason(item.startSeason)}`;
}

function buildMetaDescription(state) {
  if (state.variant === "list" && state.items.length) {
    const visibleTitles = state.items
      .slice(0, MAX_META_TITLES)
      .map((item) => item.title)
      .filter(Boolean);
    const titleSummary = visibleTitles.join(", ");
    const extraCount = Math.max(state.items.length - visibleTitles.length, 0);
    const ending = extraCount > 0 ? `, and ${extraCount} more.` : ".";
    return limitLength(
      `See Mirabellier's currently watching anime, including ${titleSummary}${ending}`,
      190,
    );
  }

  if (state.variant === "empty") {
    return "See Mirabellier's currently watching anime page. No titles are marked as currently watching right now.";
  }

  if (state.errorCode === CONFIG_ERROR_CODE) {
    return "See Mirabellier's currently watching anime page. The live MyAnimeList preview is not configured yet.";
  }

  return "See Mirabellier's currently watching anime page. The live MyAnimeList preview is unavailable right now.";
}
...
function buildAnimeImagePath(version) {
  ...
  return `/api/anime/currently-watching/embed-image.png?v=${encodedVersion}`;
}
...
async function renderListSvg(state, dimensions) {
  ...
  const contentWidth = CARD_WIDTH - CARD_PADDING_X * 2;
  const infoPillText = state.fetchedAt
    ? `Updated ${formatDateTime(state.fetchedAt)} UTC`
    : `Live snapshot`;
  ...
  return `
    <svg xmlns="http://www.w3.org/2000/svg" width="${dimensions.width}" height="${dimensions.height}" viewBox="0 0 ${dimensions.width} ${dimensions.height}">
      ...
      <circle cx="170" cy="150" r="140" fill="rgba(191,219,254,0.45)" />
      ...
      <text x="${CARD_X + CARD_PADDING_X}" y="${CARD_Y + 122}" font-family="QuicksandPreview, sans-serif" font-size="22" font-weight="700" fill="#3b82f6">Live MyAnimeList snapshot for @${escapeSvg(state.username)}</text>
      <rect x="${CARD_X + CARD_WIDTH - 400}" y="${CARD_Y + 28}" width="358" height="42" rx="21" fill="#dbeafe" />
      <text x="${CARD_X + CARD_WIDTH - 221}" y="${CARD_Y + 55}" text-anchor="middle" font-family="QuicksandPreview, sans-serif" font-size="18" font-weight="700" fill="#1d4ed8">${escapeSvg(limitLength(infoPillText, 34))}</text>
      ${bannerMarkup}
      ${rowsMarkup}
      <text x="${CARD_X + CARD_PADDING_X}" y="${dimensions.height - CARD_Y - 24}" font-family="QuicksandPreview, sans-serif" font-size="18" font-weight="700" fill="#64748b">Synced from MyAnimeList. Share this page on Discord to preview the currently watching section.</text>
    </svg>
  `;
}
