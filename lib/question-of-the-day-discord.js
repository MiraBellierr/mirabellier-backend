...
function createNotifier(db) {
  const selectQuestionByRecordedDate = db.prepare(
    `SELECT recordedDate, prompt, archivedAt, createdAt, updatedAt, discordNotifiedAt, pushNotifiedAt
  ...
  const selectActiveCarriedQuestionThroughRecordedDate = db.prepare(
    `SELECT
     ...
     GROUP BY q.recordedDate, q.prompt, q.lockedAt, q.archivedAt, q.createdAt, q.updatedAt, q.discordNotifiedAt, q.pushNotifiedAt
...
function maybeNotifyNewQuestionOfTheDayDrop(db, options = {}) {
  ...
  inFlightNotificationCheck = Promise.resolve()
    .then(async () => {
      ...
      const ws = require("./websocket-server").getWebSocketManager();
      if (ws) ws.broadcast({ type: S2C.QOTD_NEW_DAY, data: { recordedDate: activeQuestion.recordedDate } });
...
module.exports = {
  ...
  // Reused by lib/question-of-the-day-push.js so the "which question is
  // active right now" logic (including the carried-forward fallback) lives
  // in exactly one place instead of drifting between two notifiers.
