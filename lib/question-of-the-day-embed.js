...
function buildPromptLayout(prompt, carriedOver) {
  const promptTop = carriedOver ? 238 : 178;
  const promptTextTop = promptTop + 38;
  const availableHeight = 470 - promptTextTop;
  const candidates = [
    { maxChars: 60, maxLines: 4, fontSize: 28, lineHeight: 34 },
    { maxChars: 66, maxLines: 5, fontSize: 26, lineHeight: 32 },
    { maxChars: 72, maxLines: 6, fontSize: 24, lineHeight: 29 },
    { maxChars: 78, maxLines: 7, fontSize: 22, lineHeight: 27 },
    { maxChars: 84, maxLines: 8, fontSize: 20, lineHeight: 25 },
    { maxChars: 90, maxLines: 9, fontSize: 18, lineHeight: 23 },
  ];
...
function buildMainSection(state, assets) {
  ...
  return `
    <g>
      ...
      <text x="${CARD_X + 34}" y="${CARD_Y + 44}" font-family="FredokaPreview, QuicksandPreview, sans-serif" font-size="34" font-weight="700" fill="#1d4ed8">Question of the day${titleDate ? ` (${escapeSvg(titleDate)})` : ""}</text>
      ${bannerMarkup}
      <text x="${CARD_X + 34}" y="${CARD_Y + promptLayout.promptTop}" font-family="QuicksandPreview, sans-serif" font-size="17" font-weight="700" fill="#2563eb">Question:</text>
      ...
      <text x="${CARD_X + 34}" y="${CARD_Y + 426}" font-family="QuicksandPreview, sans-serif" font-size="16" font-weight="700" fill="#60a5fa">Guests can answer too. Add a name first on the live page.</text>
    </g>
  `;
}
