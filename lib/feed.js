 ...
 * sitemap's archive query: only days *before* the active carried question are
 * public (their `/question-of-the-day/archive/<date>` route resolves), while
 * the active day itself lives on /question-of-the-day and any queued later
 * prompts must never leak.
 */
function collectQuestionFeedItems(db) {
  ...
  try {
    const activeRecordedDate = db
      ...
      .get(today)?.recordedDate;
    const archiveCutoffRecordedDate = activeRecordedDate || today;
