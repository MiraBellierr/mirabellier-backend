...
function buildQuoteLayouts(quotes) {
  return quotes.map((entry, index) => {
    ...
    const lineHeight = isFeatured ? 30 : 24;
    const quoteBlockHeight =
      Math.max(lines.length, 1) * lineHeight + (isFeatured ? 78 : 66);
    ...
    .map((layout, index) => {
      ...
      return `
        <g>
          ...
          <text x="${CARD_X + CARD_PADDING_X + 18}" y="${sectionY + 70}" font-family="QuicksandPreview, sans-serif" font-size="${layout.isFeatured ? 23 : 19}" font-style="italic" font-weight="700" fill="#334155">
            ${quoteLines}
          </text>
          <text x="${CARD_X + CARD_PADDING_X + 18}" y="${sectionY + layout.height - 10}" font-family="QuicksandPreview, sans-serif" font-size="20" font-weight="700" fill="#2563eb">-- ${escapeSvg(layout.author)}</text>
        </g>
      `;
    })
    .join("");
