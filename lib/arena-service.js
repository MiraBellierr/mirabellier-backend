...
function countCollectionCards(db, userId) {
  const row = db
    .prepare(
...
function readCollectionCards(db, userId, { limit = 24, offset = 0, sort = "recent" } = {}) {
  ...
  const rows = db
    .prepare(sql)
    .all(userId, toPositiveInt(limit, 24), toPositiveInt(offset, 0));
...
function getArenaCollectionPayload(db, userId, options = {}) {
  ...
  const sort = ["recent", "rarity-desc", "rarity-asc"].includes(String(options.sort).toLowerCase())
    ? String(options.sort).toLowerCase()
    : "recent";

  const total = countCollectionCards(db, userId);
  const totalPages = Math.max(Math.ceil(total / perPage), 1);
  const cards = readCollectionCards(db, userId, { limit: perPage, offset, sort });
