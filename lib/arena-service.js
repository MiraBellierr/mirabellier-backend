...
const MAX_BANKED_WIN_DURATION = 30;
const MAX_BANKED_FIGHT_DURATION = 20;
const EFFECT_DURATION_LIMITS = Object.freeze({
  expBoostWinsRemaining: 30,
  coinBoostWinsRemaining: 30,
  drawBonusChanceWinsRemaining: 30,
  rerollKeepHigherCharges: 10,
  streakShieldCharges: 10,
  upgradeLowestRarityCharges: 10,
  guaranteeSsrPlusCharges: 10,
  fightStartShieldCharges: 20,
  evadeBoostFightsRemaining: 20,
  firstHitTrueDamageCharges: 10,
  higherRarityDamageBonusPctCharges: 10,
  gateKeyCharges: 10,
  doublePassiveTriggerFightsRemaining: 20,
});
...
function applyConsumableEffect(profile, item) {
  ...
  if (effect.kind === "exp_boost") {
    ...
    effects.drawBonusChanceWinsRemaining = Math.min(
      effects.drawBonusChanceWinsRemaining + toPositiveInt(effect.fights ?? effect.wins, 0),
      MAX_BANKED_WIN_DURATION,
    );
