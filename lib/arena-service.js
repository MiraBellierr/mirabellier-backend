...
const CARD_SHOP_MIN_PRICE = Math.min(...Object.values(CARD_SHOP_PRICES));
...
function buildArenaCardShopPayload(db, userId, offerDate) {
  ...
  return {
    ...
    randomOffer: {
      offerId: "random-card",
      minPrice: CARD_SHOP_MIN_PRICE,
      maxPrice: CARD_SHOP_MAX_PRICE,
      canBuy: profile.coins >= CARD_SHOP_MIN_PRICE,
    },
  };
}
...
async function buyArenaShopCard(db, userId, input = {}, options = {}) {
  ...
  let purchasePrice = CARD_SHOP_MAX_PRICE;
  ...
  if (kind === "daily") {
    ...
    if (currentProfile.coins < CARD_SHOP_MIN_PRICE) {
      throw new ArenaHttpError(
        400,
        "Not enough coins.",
        "ARENA_NOT_ENOUGH_COINS",
        { requiredCoins: CARD_SHOP_MIN_PRICE },
      );
    }
    await ensureArenaCardPool(db);
    const drawCard =
    ...
    purchasePrice = getCardShopPrice(purchasedCard.rarity);
  }
