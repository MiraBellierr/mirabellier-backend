...
function clearSelectedCardsForCompletedTrade(db, session, now) {
  const clearIfSelected = (ownerId, cardInstanceId) => {
    if (!ownerId || !cardInstanceId) return;
    const profile = db
    ...
    if (selectedCard?.cardInstanceId !== cardInstanceId) return;
    db.prepare("UPDATE arena_profiles SET selectedCardJson = NULL, updatedAt = ? WHERE userId = ?")
      .run(now, ownerId);
  };

  clearIfSelected(session.askerId, session.askerCardInstanceId);
  clearIfSelected(session.responderId, session.responderCardInstanceId);
}
...
function offerCardInTrade(db, userId, sessionId, cardInstanceId) {
  ...
  const tx = db.transaction(() => {
    ...
    if (session.askerId === userId) {
      db.prepare(
        `UPDATE arena_trade_sessions
         SET askerCardInstanceId = ?, askerConfirmed = 0, responderConfirmed = 0, updatedAt = ?
         WHERE id = ? AND status = 'active'`,
      ).run(normalizedCardId, now, normalizedSessionId);
    } else {
      db.prepare(
        `UPDATE arena_trade_sessions
         SET responderCardInstanceId = ?, askerConfirmed = 0, responderConfirmed = 0, updatedAt = ?
         WHERE id = ? AND status = 'active'`,
      ).run(normalizedCardId, now, normalizedSessionId);
    }
  });
...
function removeCardFromTrade(db, userId, sessionId) {
  ...
  const tx = db.transaction(() => {
    ...
    if (session.askerId === userId) {
      db.prepare(
        `UPDATE arena_trade_sessions
         SET askerCardInstanceId = NULL, askerConfirmed = 0, responderConfirmed = 0, updatedAt = ?
         WHERE id = ? AND status = 'active'`,
      ).run(now, normalizedSessionId);
    } else {
      db.prepare(
        `UPDATE arena_trade_sessions
          SET responderCardInstanceId = NULL, askerConfirmed = 0, responderConfirmed = 0, updatedAt = ?
          WHERE id = ? AND status = 'active'`,
      ).run(now, normalizedSessionId);
    }
  });
