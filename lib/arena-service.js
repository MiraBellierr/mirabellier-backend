...
function chooseEloOpponent(db, userId, randomFn = Math.random) {
  ...
  const MAX_ELO_SEARCH_EXPANSION = 3;

  for (let expansion = 0; expansion < MAX_ELO_SEARCH_EXPANSION; expansion++) {
    const candidateLimit = ELO_MATCHMAKING_CANDIDATE_LIMIT * (expansion + 1);
    const recentLimit = Math.max(2, RECENT_OPPONENT_LIMIT - expansion);
    ...
    const candidates = db
      .prepare(
        "SELECT p.* FROM arena_profiles p JOIN users u ON u.id = p.userId WHERE p.userId <> ? AND p.selectedCardJson IS NOT NULL ORDER BY ABS(COALESCE(p.eloRating, ?) - ?) ASC, COALESCE(p.eloMatches, 0) DESC, p.updatedAt ASC LIMIT ?",
      )
      .all(userId, ELO_DEFAULT_RATING, player.eloRating, candidateLimit)
    ...
    if (pool.length >= 1) {
      ...
      const normalizedRoll = Number.isFinite(roll)
        ? clamp(roll, 0, 0.999999999999)
        : 0;
      return pool[Math.floor(normalizedRoll * pool.length)] || pool[0];
    }
  }
