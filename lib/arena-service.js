...
const EFFECT_DURATION_LIMITS = Object.freeze({
  ...
  fightStartShieldCharges: 16,
  evadeBoostFightsRemaining: 16,
  firstHitTrueDamageCharges: 4,
  higherRarityDamageBonusPctCharges: 6,
  gateKeyCharges: 4,
  doublePassiveTriggerFightsRemaining: 6,
  damageBoostFightsRemaining: 20,
  speedBoostFightsRemaining: 20,
  deathSaveCharges: 4,
  statSteroidFightsRemaining: 10,
  matchRarityCharges: 4,
  vampiricHealFightsRemaining: 6,
  critChanceBoostFightsRemaining: 6,
  guardBoostFightsRemaining: 10,
  firstAttackDoubleCharges: 4,
  ivBoostCharges: 6,
  selfReviveCharges: 4,
});
...
async function simulateFight(db, input) {
  ...
  const runAttack = (attackerSide, firstActor) => {
    ...
    const outcome = calculateAttackOutcome({
      ...
      baseCritMultiplier: 0.5 + equipmentCritDmgPct / 100,
      randomFn,
    });
    ...
    if (outcome.critical && defenderRuntime.cancelCriticalCharges > 0) {
      ...
      outcome.damage = Math.max(1, Math.floor(outcome.damage / 0.5));
    ...
    if (attackerIsPlayer) {
      ...
      if (playerHp <= 0 && playerEffects.deathSaveCharges > 0) {
        playerHp = 1;
        effectUsage.usedDeathSave = true;
        pushConsole(`${playerName} survived KO with 1 HP (Phoenix Feather)`);
      }
      if (playerHp > 0 && playerEffects.selfReviveCharges > 0 && playerEffects.selfReviveHpThresholdPct > 0) {
