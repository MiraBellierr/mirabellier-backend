...
function getArenaArchivePayload(options = {}) {
  const page = Math.max(toPositiveInt(options.page, 1), 1);
  const perPage = clamp(toPositiveInt(options.perPage, 24), 1, 100);
  const search = String(options.search || "").trim();
  ...
  const tx = db.transaction(() => {
    ...
    const createSession = (sessionCardInstanceId, sessionCardOwnerId) => {
      ...
      db.prepare(
        ...
        sessionCardOwnerId === row.askerId && sessionCardInstanceId
          ? serializeTradeCardIds([sessionCardInstanceId])
          : null,
        sessionCardOwnerId === userId && sessionCardInstanceId
          ? serializeTradeCardIds([sessionCardInstanceId])
          : null,
        now, now,
      );
