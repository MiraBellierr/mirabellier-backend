...
function countCollectionCards(db, userId, search = "", element = "", ivFilters = {}) {
  ...
  if (minLuck > 0) {
    conditions.push("CAST(json_extract(cardJson, '$.iv.luck') AS INTEGER) >= ?");
    params.push(minLuck);
  }
...
function readCollectionCards(db, userId, { limit = 24, offset = 0, sort = "recent", search = "", element = "", minPower = 0, minGuard = 0, minSpeed = 0, minLuck = 0 } = {}) {
  ...
  } else if (sort === "luck-desc") {
    orderBy = "CAST(json_extract(cardJson, '$.iv.luck') AS INTEGER) DESC, createdAt DESC";
  }
  ...
  pieceRows.forEach((row) => {
    ...
    subStatsArray.forEach((s) => {
      ...
      switch (s.type) {
        ...
        case "luck": flatStats.luck += val; break;
...
function getArenaCollectionPayload(db, userId, options = {}) {
  ...
  const validSorts = ["recent", "rarity-desc", "rarity-asc", "iv-desc", "iv-asc", "RH", "RL", "IH", "IL", "power-desc", "guard-desc", "speed-desc", "luck-desc"];
  const sort = validSorts.includes(rawSort) ? rawSort : "recent";
  const search = String(options.search || "").trim();
  const element = String(options.element || "").trim();

  const total = countCollectionCards(db, userId, search, element);
  const totalPages = Math.max(Math.ceil(total / perPage), 1);
  const cards = readCollectionCards(db, userId, { limit: perPage, offset, sort, search, element });
...
function mintRainbowCard(db, userId, id1, id2) {
  ...
  if (card1.rainbow || card2.rainbow) {
    throw new ArenaHttpError(400, "Rainbow cards cannot be used as minting material.", "ARENA_MINT_ALREADY_RAINBOW");
  }
  ...
  const tradeSessionCheck = db.prepare(
    "SELECT id FROM arena_trade_sessions WHERE (askerCardInstanceId IN (?, ?) OR responderCardInstanceId IN (?, ?)) AND status = 'active' LIMIT 1",
  ).get(card1Id, card2Id, card1Id, card2Id);
  if (tradeSessionCheck) {
    throw new ArenaHttpError(409, "A card is currently in an active trade session.", "ARENA_MINT_CARD_TRADED");
  }

  // Use the card with the higher total IV as the base, then randomly distribute 3 bonus points (capped at CARD_IV_MAX)
  const betterCard = card1.iv.total >= card2.iv.total ? card1 : card2;
  const baseIv = {
    power: betterCard.iv.power,
    guard: betterCard.iv.guard,
    speed: betterCard.iv.speed,
    luck: betterCard.iv.luck,
  };
  ...
  mintedIv.total = mintedIv.power + mintedIv.guard + mintedIv.speed + mintedIv.luck;
...
function createArenaTradeListing(db, userId, input = {}) {
  ...
  const tx = db.transaction(() => {
    ...
    const alreadyInSession = db
      .prepare(
        `SELECT id
         FROM arena_trade_sessions
         WHERE (askerCardInstanceId = ? OR responderCardInstanceId = ?)
           AND status = 'active'
         LIMIT 1`,
      )
      .get(cardInstanceId, cardInstanceId);
    if (alreadyInSession) {
      throw new ArenaHttpError(
        409,
    ...
    db.prepare(
      `INSERT INTO arena_trade_listings (
        ...
        wantedRarity, wantedElement, note, status,
...
function sendTradeRequest(db, askerId, responderId, cardInstanceId) {
  if (!responderId) {
    throw new ArenaHttpError(
      400,
  ...
  const tx = db.transaction(() => {
    ...
    db.prepare(
      `INSERT INTO arena_trade_requests (
        id, askerId, responderId, askerCardInstanceId,
        status, createdAt, updatedAt, respondedAt, cancelledAt
      ) VALUES (?, ?, ?, ?, 'pending', ?, ?, NULL, NULL)`,
    ).run(requestId, askerId, responderId, cardInstanceId || null, now, now);
    ...
    if (cardInstanceId) {
      ...
      if (cardRow) {
        ...
        if (card) {
          // Look up the responder's listed card
          const listedRow = db
...
function acceptTradeRequest(db, userId, requestId) {
  ...
  const tx = db.transaction(() => {
    ...
    // Direct trade request (no listing, no offered card): create a session
    if (!listingRow && !row.askerCardInstanceId) {
      ...
      db.prepare(
        `INSERT INTO arena_trade_sessions (id, requestId, askerId, responderId, status, createdAt, updatedAt)
         VALUES (?, ?, ?, ?, 'active', ?, ?)`,
      ).run(sessionId, normalizedId, row.askerId, userId, now, now);
    ...
    if (!listingRow) {
      throw new ArenaHttpError(
        409,
        "No active trade listing found. The card may have already been traded.",
        "ARENA_TRADE_NO_LISTING",
      );
    }
    ...
    if (!responderCard) {
      throw new ArenaHttpError(
        409,
        "Your listing card data is invalid.",
        "ARENA_TRADE_INVALID_LISTING_CARD",
      );
    }
    ...
    if (!row.askerCardInstanceId) {
      throw new ArenaHttpError(
        400,
        "No card was offered by the asker.",
    ...
    // Cancel other pending requests for this responder (listing is being removed)
    db.prepare(
      `UPDATE arena_trade_requests
       SET status = 'cancelled', cancelledAt = ?, updatedAt = ?
    ...
    db.prepare(
      `UPDATE arena_trade_sessions
       SET status = 'cancelled', updatedAt = ?
       WHERE (askerId = ? OR responderId = ?) AND status = 'active'`,
    ).run(now, userId, userId);

    // Atomic swap:
    // Remove asker's card from their collection
    db.prepare(
      `DELETE FROM arena_card_collection
       WHERE userId = ? AND cardInstanceId = ?`,
    ).run(row.askerId, row.askerCardInstanceId);

    // Remove the listing (card was in escrow)
    db.prepare(
      `UPDATE arena_trade_listings
       SET status = 'cancelled', cancelledAt = ?, updatedAt = ?
       WHERE id = ? AND status = 'active'`,
    ...
    // Give cards to new owners
    insertCollectionCard(db, userId, askerCard);
    insertCollectionCard(db, row.askerId, responderCard);

    // Clear active card selection if either swapped out their selected card
    const responderProfile = db.prepare("SELECT selectedCardJson FROM arena_profiles WHERE userId = ? LIMIT 1").get(userId);
    if (responderProfile?.selectedCardJson) {
      try {
        const selectedCard = JSON.parse(responderProfile.selectedCardJson);
        if (selectedCard?.cardInstanceId === responderCard.cardInstanceId) {
          db.prepare("UPDATE arena_profiles SET selectedCardJson = NULL, updatedAt = ? WHERE userId = ?").run(now, userId);
        }
      } catch { /* ignore */ }
    }
    const askerProfile = db.prepare("SELECT selectedCardJson FROM arena_profiles WHERE userId = ? LIMIT 1").get(row.askerId);
    if (askerProfile?.selectedCardJson) {
      try {
        const selectedCard = JSON.parse(askerProfile.selectedCardJson);
        if (selectedCard?.cardInstanceId === row.askerCardInstanceId) {
          db.prepare("UPDATE arena_profiles SET selectedCardJson = NULL, updatedAt = ? WHERE userId = ?").run(now, row.askerId);
        }
      } catch { /* ignore */ }
    }
    ...
    // Notify asker: they received the responder's card
    createArenaNotification(
      db, row.askerId, "trade_accepted",
      `${responderName} accepted your trade request!`,
      `You received: ${responderCard.title}.`,
      "/arena/inbox",
      JSON.stringify({ askerCard, responderCard }),
    );

    // Notify responder: they received the asker's card
    createArenaNotification(
      db, userId, "trade_accepted",
      `You accepted ${askerName} trade request!`,
      `You received: ${askerCard.title}.`,
      "/arena/inbox",
      JSON.stringify({ askerCard: responderCard, responderCard: askerCard }),
    );

    // Update the responder's original trade_request notification title
    const responderNotifRow = db.prepare(
      `SELECT id FROM arena_notifications
       WHERE userId = ? AND type = 'trade_request' AND metadata LIKE '%' || ? || '%'
       ORDER BY createdAt DESC LIMIT 1`,
    ...
    _notifyUser(row.askerId, S2C.ARENA_TRADE_REQUEST_UPDATE, { requestId: normalizedId, status: "accepted" });
    return { askerCard, responderCard };
  });
...
function denyTradeRequest(db, userId, requestId) {
  ...
  const tx = db.transaction(() => {
    ...
    const responderListedRow = db.prepare("SELECT cardJson FROM arena_trade_listings WHERE userId = ? AND status = 'active' LIMIT 1")
      .get(userId);
...
function getTradeSession(db, userId, sessionId) {
  ...
  // Auto-cancel if either participant no longer has their placed card
  if (row.status === "active") {
    ...
    if (row.responderCardInstanceId) {
      const responderOwns = db
        .prepare("SELECT id FROM arena_card_collection WHERE userId = ? AND cardInstanceId = ? LIMIT 1")
        .get(row.responderId, row.responderCardInstanceId);
  ...
  let askerCard = null;
  let responderCard = null;
  if (row.askerCardInstanceId) {
    const ownerId = row.status === "completed" ? row.responderId : row.askerId;
    const askerRow = db
      .prepare(
        `SELECT cardJson
         FROM arena_card_collection
      ...
      .get(ownerId, row.askerCardInstanceId);
    if (askerRow) {
      askerCard = normalizeSelectedCard(askerRow.cardJson);
    }
  }
  if (row.responderCardInstanceId) {
    const ownerId = row.status === "completed" ? row.askerId : row.responderId;
    const responderRow = db
      .prepare(
        `SELECT cardJson
         FROM arena_card_collection
         WHERE userId = ? AND cardInstanceId = ?
      ...
      .get(ownerId, row.responderCardInstanceId);
    if (responderRow) {
      responderCard = normalizeSelectedCard(responderRow.cardJson);
    }
  }
...
function offerCardInTrade(db, userId, sessionId, cardInstanceId) {
  ...
  const tx = db.transaction(() => {
    ...
    const otherCardInSession = db
      .prepare(
        `SELECT id
         FROM arena_trade_sessions
         WHERE id != ? AND status = 'active'
           AND (askerCardInstanceId = ? OR responderCardInstanceId = ?)
         LIMIT 1`,
      )
      .get(normalizedSessionId, normalizedCardId, normalizedCardId);
    ...
    if (session.askerId === userId) {
      db.prepare(
        `UPDATE arena_trade_sessions
         SET askerCardInstanceId = ?, askerConfirmed = 0, updatedAt = ?
         WHERE id = ? AND status = 'active'`,
      ).run(normalizedCardId, now, normalizedSessionId);
    } else {
      db.prepare(
        `UPDATE arena_trade_sessions
         SET responderCardInstanceId = ?, responderConfirmed = 0, updatedAt = ?
         WHERE id = ? AND status = 'active'`,
      ).run(normalizedCardId, now, normalizedSessionId);
    }
  });
...
function removeCardFromTrade(db, userId, sessionId) {
  const normalizedSessionId = String(sessionId || "").trim();
  const tx = db.transaction(() => {
    ensureArenaProfile(db, userId);
    const session = db
    ...
    if (session.askerId === userId) {
      db.prepare(
        `UPDATE arena_trade_sessions
         SET askerCardInstanceId = NULL, askerConfirmed = 0, updatedAt = ?
         WHERE id = ? AND status = 'active'`,
      ).run(now, normalizedSessionId);
    } else {
      db.prepare(
        `UPDATE arena_trade_sessions
          SET responderCardInstanceId = NULL, responderConfirmed = 0, updatedAt = ?
          WHERE id = ? AND status = 'active'`,
      ).run(now, normalizedSessionId);
    }
  });
...
function confirmTrade(db, userId, sessionId) {
  ...
  const tx = db.transaction(() => {
    ...
    const userCardInstanceId = isAsker ? session.askerCardInstanceId : session.responderCardInstanceId;
    if (!userCardInstanceId) {
      throw new ArenaHttpError(
        400,
        "Place a card in your slot before confirming.",
        "ARENA_TRADE_NO_CARD",
      );
    }
    ...
    if (updated.askerConfirmed === 1 && updated.responderConfirmed === 1) {
      if (!updated.askerCardInstanceId || !updated.responderCardInstanceId) {
        throw new ArenaHttpError(
          409,
          "Both players must have a card placed.",
          "ARENA_TRADE_MISSING_CARDS",
        );
      }

      // Atomic swap: remove each card from its owner and insert into the other
      const askerCardRow = db
        .prepare(
          `SELECT cardJson, id
           FROM arena_card_collection
        ...
        .get(updated.askerId, updated.askerCardInstanceId);
      const responderCardRow = db
        .prepare(
          `SELECT cardJson, id
           FROM arena_card_collection
           WHERE userId = ? AND cardInstanceId = ?
        ...
        .get(updated.responderId, updated.responderCardInstanceId);

      if (!askerCardRow || !responderCardRow) {
        throw new ArenaHttpError(
          409,
      ...
      const askerCard = normalizeSelectedCard(askerCardRow.cardJson);
      const responderCard = normalizeSelectedCard(responderCardRow.cardJson);

      // Remove cards from original owners
      db.prepare(
        `DELETE FROM arena_card_collection
         WHERE userId = ? AND cardInstanceId = ?`,
      ).run(updated.askerId, updated.askerCardInstanceId);
      db.prepare(
        `DELETE FROM arena_card_collection
         WHERE userId = ? AND cardInstanceId = ?`,
      ).run(updated.responderId, updated.responderCardInstanceId);

      // Give asker's card to responder and vice versa
      insertCollectionCard(db, updated.responderId, askerCard);
      insertCollectionCard(db, updated.askerId, responderCard);

      // Clear selected card if either swapped out their active card
      if (profile.selectedCard?.cardInstanceId === updated.askerCardInstanceId && userId === updated.askerId) {
        db.prepare(
          `UPDATE arena_profiles
           SET selectedCardJson = NULL, updatedAt = ?
           WHERE userId = ?`,
      ...
      if (profile.selectedCard?.cardInstanceId === updated.responderCardInstanceId && userId === updated.responderId) {
        db.prepare(
          `UPDATE arena_profiles
           SET selectedCardJson = NULL, updatedAt = ?
           WHERE userId = ?`,
...
function chooseEloOpponent(db, userId, randomFn = Math.random) {
  ...
  const recentOpponentIds = new Set(
    db
      .prepare(
        `SELECT opponentUserId FROM arena_fights WHERE userId = ? AND opponentUserId NOT LIKE 'npc:%' ORDER BY createdAt DESC LIMIT ?`,
      )
  ...
  const candidates = db
    .prepare(
      `SELECT p.* FROM arena_profiles p JOIN users u ON u.id = p.userId WHERE p.userId <> ? AND p.selectedCardJson IS NOT NULL ORDER BY ABS(COALESCE(p.eloRating, ?) - ?) ASC, COALESCE(p.eloMatches, 0) DESC LIMIT ?`,
    )
    .all(userId, ELO_DEFAULT_RATING, player.eloRating, ELO_MATCHMAKING_CANDIDATE_LIMIT * 2)
    .map((row) => mapArenaProfileRow(row))
    .filter(Boolean);

  if (candidates.length === 0) return null;

  const freshCandidates = candidates.filter(
    (c) => !recentOpponentIds.has(c.userId),
  );
  const pool = (freshCandidates.length > 0 ? freshCandidates : candidates)
    .slice(0, ELO_MATCHMAKING_POOL_SIZE);

  if (pool.length === 0) return null;

  const weights = pool.map((c) => {
    const diff = Math.abs((c.eloRating || ELO_DEFAULT_RATING) - (player.eloRating || ELO_DEFAULT_RATING));
    return 1 / (1 + diff / 50);
  });
...
const NPC_TEMPLATES = [
  ...
  {
    id: "forest-wisp",
    displayName: "Forest Wisp",
    level: 3,
    rarity: "C",
    ivRange: { min: 8, max: 18 },
    statScale: 0.80,
  },
  {
    id: "iron-squire",
    displayName: "Iron Squire",
    level: 4,
    rarity: "C",
...
async function buildNpcOpponent(db, playerLevel = 1) {
  const template =
    NPC_TEMPLATES.find((t) => t.level === Math.min(playerLevel, 5)) ||
    NPC_TEMPLATES[0];
  ...
  const levelDelta = template.level - 1;
  const baseStats = {
    ...
    luck: BASE_PROFILE.luck + LEVEL_UP_GAINS.luck * levelDelta,
  };
  ...
  return {
    ...
    luck: Math.floor(baseStats.luck * template.statScale),
    lifetimeCoinsEarned: 0,
    eloRating: 600 + Math.floor(Math.random() * 400),
    eloMatches: 0,
...
async function selectOpponentForFight(db, userId) {
  ...
  // NPC fallback only for levels 1-4
  if (player.level < 5) {
    const npc = await buildNpcOpponent(db, player.level);
    return { profile: npc, isNpc: true, displayName: npc.displayName };
  }
  ...
  throw new ArenaHttpError(
    409, "ARENA_NO_OPPONENT",
    "No opponents available. Try again later.",
  );
}

async function simulateFight(db, input) {
  const { player, opponent, playerEffects: rawEffects, randomFn = Math.random } = input;
  ...
  if (playerEffects.statSteroidFightsRemaining > 0 && playerEffects.statSteroidPct > 0) {
    ...
    playerTotalStats.luck = Math.floor(playerTotalStats.luck * pct);
    ...
    playerBaseStats.luck = Math.floor(playerBaseStats.luck * pct);
  ...
  if (playerEffects.ivBoostCharges > 0) {
    ...
    const ivStats = ["power", "guard", "speed", "luck"];
    const totalBoost = 5;
    ...
    playerTotalStats.luck += statDelta("luck");
    ...
    playerBaseStats.luck += statDelta("luck");
  ...
  const runAttack = (attackerSide, firstActor) => {
    ...
    outcome.damage = Math.max(1, Math.floor(outcome.damage * elementMult));
    ...
    if (attackerIsPlayer) {
      ...
      // Death save and self-revive for player
...
async function runFight(db, userId) {
  ...
  return {
    ...
    opponent: {
      ...
      eloRating: opponentSelection.isNpc
        ? null
        : opponentSnapshot.profile.eloRating,
      eloMatches: opponentSelection.isNpc
        ? 0
        : opponentSnapshot.profile.eloMatches,
      eloProvisional: opponentSelection.isNpc
        ? false
        : isEloProvisional(opponentSnapshot.profile.eloMatches),
...
function buildArenaCardShopPayload(db, userId, offerDate) {
  ...
  return {
    ...
    randomOffer: isRandomCardOfferAvailable(offerDate)
      ? {
          offerId: "random-card",
...
async function buyArenaShopCard(db, userId, input = {}, options = {}) {
  ...
  if (kind === "daily") {
    ...
    if (!isRandomCardOfferAvailable(offerDate)) {
      throw new ArenaHttpError(
        410,
        "Random card is only available on Saturday, Sunday, Tuesday, and Thursday.",
  ...
  const tx = db.transaction(() => {
    ...
    db.prepare(
      ...
      JSON.stringify({
        ...
        eloRating: opponentSelection.isNpc
          ? null
          : opponentSelection.profile.eloRating,
        eloMatches: opponentSelection.isNpc
          ? 0
          : opponentSelection.profile.eloMatches,
        eloProvisional: opponentSelection.isNpc
          ? false
          : isEloProvisional(opponentSelection.profile.eloMatches),
        stats: opponentSnapshot.totalStats,
        equipment: opponentSnapshot.equipped,
        selectedCard: opponentSnapshot.selectedCard,
      }),
