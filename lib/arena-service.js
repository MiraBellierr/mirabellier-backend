...
function chooseEloOpponent(db, userId, randomFn = Math.random) {
  ...
  // Get candidates within ±200 ELO, ordered by proximity
  const candidates = db
    .prepare(
      "SELECT p.* FROM arena_profiles p JOIN users u ON u.id = p.userId WHERE p.userId <> ? AND p.selectedCardJson IS NOT NULL AND ABS(COALESCE(p.eloRating, ?) - ?) <= ? ORDER BY ABS(COALESCE(p.eloRating, ?) - ?) ASC, COALESCE(p.eloMatches, 0) DESC LIMIT ?",
    )
    .all(userId, ELO_DEFAULT_RATING, player.eloRating, ELO_RANGE, ELO_DEFAULT_RATING, player.eloRating, ELO_MATCHMAKING_CANDIDATE_LIMIT * 2)
