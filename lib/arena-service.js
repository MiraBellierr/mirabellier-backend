...
const CARD_SHOP_RANDOM_END_DATE = "2026-06-23";
...
function isRandomCardOfferAvailable(recordedDate = getCurrentRecordedDate()) {
  return String(recordedDate || "") < CARD_SHOP_RANDOM_END_DATE;
}
...
function countCollectionCards(db, userId, search = "", element = "") {
...
function readCollectionCards(db, userId, { limit = 24, offset = 0, sort = "recent", search = "", element = "" } = {}) {
...
function getArenaCollectionPayload(db, userId, options = {}) {
  ...
  const validSorts = ["recent", "rarity-desc", "rarity-asc", "iv-desc", "iv-asc", "RH", "RL", "IH", "IL"];
  ...
  return {
    ...
    randomOffer: isRandomCardOfferAvailable(offerDate)
      ? {
          ...
          endsAt: `${CARD_SHOP_RANDOM_END_DATE}T00:00:00.000Z`,
        }
      : null,
  };
}
...
async function buyArenaShopCard(db, userId, input = {}, options = {}) {
  ...
  if (kind === "daily") {
    ...
    if (!isRandomCardOfferAvailable(offerDate)) {
      throw new ArenaHttpError(
        410,
        "The random card offer has ended.",
        "ARENA_RANDOM_CARD_OFFER_ENDED",
        { endedAt: `${CARD_SHOP_RANDOM_END_DATE}T00:00:00.000Z` },
      );
    }
