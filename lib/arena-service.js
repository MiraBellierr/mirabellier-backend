...
function acceptTradeRequest(db, userId, requestId) {
  ...
  const tx = db.transaction(() => {
    ...
    const createSession = (sessionCardInstanceId, sessionCardOwnerId) => {
      ...
      db.prepare(
        `INSERT INTO arena_trade_sessions (id, requestId, askerId, responderId,
           askerCardInstanceId, responderCardInstanceId, status, createdAt, updatedAt)
         VALUES (?, ?, ?, ?, ?, ?, 'active', ?, ?)`,
      ).run(
    ...
    // Case D: Both have cards → immediate swap (legacy)
    const responderCard = normalizeSelectedCard(listingRow.cardJson);
    if (!responderCard) {
      throw new ArenaHttpError(409, "Your listing card data is invalid.", "ARENA_TRADE_INVALID_LISTING_CARD");
    }
...
function offerCoinInTrade(db, userId, sessionId, amount) {
  ...
  const tx = db.transaction(() => {
    ...
    if (session.askerId === userId) {
      db.prepare(
        `UPDATE arena_trade_sessions
         SET askerCoins = ?, askerConfirmed = 0, updatedAt = ?
         WHERE id = ? AND status = 'active'`,
      ).run(coinAmount, now, normalizedSessionId);
    } else {
      db.prepare(
        `UPDATE arena_trade_sessions
         SET responderCoins = ?, responderConfirmed = 0, updatedAt = ?
         WHERE id = ? AND status = 'active'`,
      ).run(coinAmount, now, normalizedSessionId);
    }
  });
...
function removeCoinFromTrade(db, userId, sessionId) {
  ...
  const tx = db.transaction(() => {
    ...
    if (session.askerId === userId) {
      db.prepare(
        `UPDATE arena_trade_sessions
         SET askerCoins = 0, askerConfirmed = 0, updatedAt = ?
         WHERE id = ? AND status = 'active'`,
      ).run(now, normalizedSessionId);
    } else {
      db.prepare(
        `UPDATE arena_trade_sessions
         SET responderCoins = 0, responderConfirmed = 0, updatedAt = ?
         WHERE id = ? AND status = 'active'`,
      ).run(now, normalizedSessionId);
    }
  });
...
function confirmTrade(db, userId, sessionId) {
  ...
  const tx = db.transaction(() => {
    ...
    // Validate card ownership if a card is placed
    ...
    if (updated.askerConfirmed === 1 && updated.responderConfirmed === 1) {
      const askerHasCard = !!updated.askerCardInstanceId;
      const responderHasCard = !!updated.responderCardInstanceId;
      ...
      // Transfer cards
      if (askerHasCard && responderHasCard) {
        // Card-for-card swap (existing logic)
        const askerCardRow = db
          .prepare(
            `SELECT cardJson, id
             FROM arena_card_collection
      ...
      } else if (askerHasCard && !responderHasCard) {
        // Asker gives card to responder (no card back)
        const askerCardRow = db
          .prepare(
            `SELECT cardJson, id
             FROM arena_card_collection
        ...
        if (!askerCardRow) {
          throw new ArenaHttpError(
            409,
            "Your card is no longer available.",
            "ARENA_TRADE_CARD_UNAVAILABLE",
          );
        }
        ...
        // Responder gives card to asker (no card back)
        const responderCardRow = db
          .prepare(
            `SELECT cardJson, id
             FROM arena_card_collection
