...
const MAX_BANKED_WIN_DURATION = 25;
const MAX_BANKED_FIGHT_DURATION = 20;
const EFFECT_DURATION_LIMITS = Object.freeze({
  expBoostWinsRemaining: 10,
  coinBoostWinsRemaining: 10,
  drawBonusChanceWinsRemaining: 10,
  rerollKeepHigherCharges: 1,
  streakShieldCharges: 2,
  upgradeLowestRarityCharges: 1,
  guaranteeSsrPlusCharges: 1,
  fightStartShieldCharges: 5,
  evadeBoostFightsRemaining: 5,
  firstHitTrueDamageCharges: 1,
  higherRarityDamageBonusPctCharges: 1,
  gateKeyCharges: 1,
  doublePassiveTriggerFightsRemaining: 3,
});
...
function readCollectionCards(db, userId, { limit = 24, offset = 0, sort = "recent", search = "" } = {}) {
  ...
  if (sort === "rarity-desc" || sort === "RH") {
    orderBy = "cardJson DESC, createdAt DESC";
  } else if (sort === "rarity-asc" || sort === "RL") {
    orderBy = "cardJson ASC, createdAt DESC";
