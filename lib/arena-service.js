...
const EFFECT_DURATION_LIMITS = Object.freeze({
  ...
  higherRarityDamageBonusPctCharges: 4,
  gateKeyCharges: 4,
...
function countCollectionCards(db, userId, search = "") {
  ...
  const row = db
    .prepare(
      `SELECT COUNT(*) AS total
       FROM arena_card_collection
       WHERE userId = ?
         AND (LOWER(json_extract(cardJson, '$.title')) LIKE LOWER(?)
...
function readCollectionCards(db, userId, { limit = 24, offset = 0, sort = "recent", search = "" } = {}) {
  ...
  if (term) {
    ...
    sql = `SELECT cardJson
     ...
     ORDER BY ${orderBy}
     LIMIT ? OFFSET ?`;
    params = [userId, like, like, like, toPositiveInt(limit, 24), toPositiveInt(offset, 0)];
  } else {
    sql = `SELECT cardJson
     FROM arena_card_collection
     WHERE userId = ?
     ORDER BY ${orderBy}
     LIMIT ? OFFSET ?`;
    params = [userId, toPositiveInt(limit, 24), toPositiveInt(offset, 0)];
  }
...
function getArenaCollectionPayload(db, userId, options = {}) {
  ...
  const total = countCollectionCards(db, userId, search);
  const totalPages = Math.max(Math.ceil(total / perPage), 1);
  const cards = readCollectionCards(db, userId, { limit: perPage, offset, sort, search });
...
function applyConsumableEffect(profile, item) {
  ...
  if (effect.kind === "exp_boost") {
    ...
    const fights = toPositiveInt(effect.fights ?? effect.wins, 0);
    ...
    const fights = toPositiveInt(effect.fights ?? effect.wins, 0);
    effects.coinBoostWinsRemaining = Math.min(
      effects.coinBoostWinsRemaining + fights,
      Math.max(effects.coinBoostWinsRemaining, fights * 2),
    );
  } else if (effect.kind === "draw_bonus_chance") {
    effects.drawBonusChancePct = Math.max(effects.drawBonusChancePct, toPositiveInt(effect.pct, 0));
    const fights = toPositiveInt(effect.fights ?? effect.wins, 0);
    effects.drawBonusChanceWinsRemaining = Math.min(
      effects.drawBonusChanceWinsRemaining + fights,
      Math.max(effects.drawBonusChanceWinsRemaining, fights * 2),
    );
  } else if (effect.kind === "reroll_keep_higher") {
    const charges = toPositiveInt(effect.charges, 1);
    effects.rerollKeepHigherCharges = Math.min(
      effects.rerollKeepHigherCharges + charges,
      Math.max(effects.rerollKeepHigherCharges, charges * 2),
    );
  } else if (effect.kind === "streak_shield") {
    const charges = toPositiveInt(effect.charges, 1);
    effects.streakShieldCharges = Math.min(
      effects.streakShieldCharges + charges,
      Math.max(effects.streakShieldCharges, charges * 2),
    );
  } else if (effect.kind === "upgrade_lowest_rarity") {
    const charges = toPositiveInt(effect.charges, 1);
    effects.upgradeLowestRarityCharges = Math.min(
      effects.upgradeLowestRarityCharges + charges,
      Math.max(effects.upgradeLowestRarityCharges, charges * 2),
    );
  } else if (effect.kind === "guarantee_ssr_plus") {
    const charges = toPositiveInt(effect.charges, 1);
    effects.guaranteeSsrPlusCharges = Math.min(
      effects.guaranteeSsrPlusCharges + charges,
      Math.max(effects.guaranteeSsrPlusCharges, charges * 2),
    );
    ...
    const charges = toPositiveInt(effect.charges, 1);
    effects.gateKeyCharges = Math.min(
      effects.gateKeyCharges + charges,
      Math.max(effects.gateKeyCharges, charges * 2),
    );
