...
function getArenaProfilePayload(db, userId) {
  ...
  const activePassives = resolveActivePassives(equippedRows);
...
function tierIndexForLevel(level) {
  const numeric = toInt(level, 1);
  let index = 0;
  SHOP_TIERS.forEach((tier, tierIndex) => {
    const unlock = toInt(TIER_UNLOCK_LEVELS[tier], 1);
    if (numeric >= unlock) index = tierIndex;
  });
  return index;
}

function selectMaterialDropTierIndex(playerLevel, opponentLevel, randomFn = Math.random) {
  const cappedTier = Math.min(
    tierIndexForLevel(playerLevel),
    tierIndexForLevel(opponentLevel),
  );
  const weights = [];
  let total = 0;
...
function rollMaterialDrops(playerLevel, opponentLevel, dropCount, randomFn = Math.random) {
  const drops = [];
  for (let index = 0; index < dropCount; index += 1) {
    const tierIndex = selectMaterialDropTierIndex(playerLevel, opponentLevel, randomFn);
    const pool = MATERIAL_IDS_BY_TIER[tierIndex] || MATERIAL_IDS_BY_TIER[0] || [];
    if (pool.length === 0) continue;
    const selected = pool[randomInt(0, pool.length - 1, randomFn)];
    drops.push(selected);
  }
  return drops;
}
...
function calculateAttackOutcome(input) {
  ...
  damage += toInt(attackerTrueDamage, 0);
...
function runPassivesForTrigger(input) {
  ...
  passives.forEach((passive) => {
    ...
    actions.forEach((action, actionIndex) => {
      ...
      if (chancePct < 100 && randomFn() * 100 >= chancePct) return;
      ...
      if (type === "addFlatDamage") {
        ...
        const healAmount = Math.max(toInt(value, 0), 0);
        if (healAmount > 0) {
          context.self.hp = Math.min(context.self.maxHp, context.self.hp + healAmount);
        }
        ...
        mods.extraStrikeChancePct = Math.max(mods.extraStrikeChancePct, chancePct);
...
async function simulateFight(db, input) {
  ...
  if (playerEffects.higherRarityDamageBonusPctCharges > 0) {
    effectUsage.usedHigherRarityBonus = true;
  }
  ...
  const runAttack = (attackerSide, firstActor) => {
    ...
    if (
      ...
      trueDamage += playerEffects.firstHitTrueDamageValue;
    ...
    if (
      onDamageDealtMods.extraStrikeChancePct > 0 &&
      randomFn() * 100 < onDamageDealtMods.extraStrikeChancePct
    ) {
    ...
    if (finalDamage > 0) {
      pushConsole(`${defenderName} took ${finalDamage} HP`);
    } else {
      pushConsole(`${defenderName} took 0 HP`);
    }
...
async function runFight(db, userId) {
  ...
  const tx = db.transaction(() => {
    ...
    const materialDropCount = simulation.playerWon ? 2 : 1;
    const materialDrops = rollMaterialDrops(
      current.level,
      opponentSnapshot.profile.level,
      materialDropCount,
    );
    materialDrops.forEach((itemId) => {
      upsertInventoryItem(db, current.userId, itemId, 1);
    });
    const materialDropSummary = {};
    materialDrops.forEach((itemId) => {
      materialDropSummary[itemId] = (materialDropSummary[itemId] || 0) + 1;
    });
    ...
    return {
      ...
      coinDelta,
      rarityCoinReward,
      materialDrops: Object.entries(materialDropSummary).map(([itemId, quantity]) => ({
        itemId,
