...
function chooseEloOpponent(db, userId, randomFn = Math.random) {
  ...
  const ELO_EXPANSIONS = [1, 2, 3]; // 200 → 400 → 600

  for (const multiplier of ELO_EXPANSIONS) {
    const range = BASE_RANGE * multiplier;
    const candidates = db
      .prepare(
      ...
      .all(userId, ELO_DEFAULT_RATING, player.eloRating, range, ELO_DEFAULT_RATING, player.eloRating, ELO_MATCHMAKING_CANDIDATE_LIMIT * 2)
