...
function rarityFromFavorites(favorites, randomFn = Math.random) {
  ...
  if (numeric >= CHARACTER_FAVORITES_UR_MIN) {
    return "UR";
  }

  const clampedFav = clamp(numeric, CHARACTER_FAVORITES_MIN, CHARACTER_FAVORITES_UR_MIN);
  const low = Math.log(CHARACTER_FAVORITES_MIN);
  const high = Math.log(CHARACTER_FAVORITES_UR_MIN);
  const progress = high > low ? (Math.log(clampedFav) - low) / (high - low) : 0;

  for (const band of RARITY_PROGRESS_BANDS) {
    if (band.rarity === "UR") break;
  ...
  return {
    ...
    dailyOffers: offers.map((offer) => {
      ...
      return {
        ...
        price: CARD_SHOP_PRICE,
        sold,
        canBuy: !sold && profile.coins >= CARD_SHOP_PRICE,
      };
    }),
    randomOffer: {
      offerId: "random-card",
      price: CARD_SHOP_PRICE,
      canBuy: profile.coins >= CARD_SHOP_PRICE,
    },
  };
}
...
async function buyArenaShopCard(db, userId, input = {}, options = {}) {
  ...
  if (currentProfile.coins < CARD_SHOP_PRICE) {
    throw new ArenaHttpError(
      400,
      "Not enough coins.",
  ...
  const tx = db.transaction(() => {
    const profile = ensureArenaProfile(db, userId);
    if (profile.coins < CARD_SHOP_PRICE) {
      throw new ArenaHttpError(
        400,
        "Not enough coins.",
        "ARENA_NOT_ENOUGH_COINS",
    ...
    db.prepare(
      `UPDATE arena_profiles
       SET coins = ?, updatedAt = ?
       WHERE userId = ?`,
    ).run(profile.coins - CARD_SHOP_PRICE, updatedAt, userId);
