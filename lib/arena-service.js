...
function canDrawDailyCard(profile) {
  ...
  return profile.lastCardDrawDate !== today;
}
...
function toPublicProfile(profile, equipmentStats, equippedItems, options = {}) {
  ...
  const canDrawCard = canDrawDailyCard(profile);
...
async function drawDailyCard(db, userId) {
  ...
  const tx = db.transaction(() => {
    const profile = ensureArenaProfile(db, userId);
    if (profile.lastCardDrawDate === today) {
      throw new ArenaHttpError(
        409,
        "You can only draw one card per day.",
    ...
    db.prepare(
      `UPDATE arena_profiles
       SET selectedCardJson = ?, lastCardDrawDate = ?, updatedAt = ?
       WHERE userId = ?`,
    ).run(serializeSelectedCard(drawnCard), today, nowIso(), userId);
