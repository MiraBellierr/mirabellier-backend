...
function chooseEloOpponent(db, userId, randomFn = Math.random) {
  ...
  // Get a generous pool of ELO-close candidates
  const candidates = db
    .prepare(
  ...
  // Prefer opponents within ±5 levels when available
  const levelBracket = candidates.filter(
    (c) => Math.abs((c.level || 1) - (player.level || 1)) <= 5,
  );
  const bracketCandidates = levelBracket.length >= 3 ? levelBracket : candidates;

  // Filter out recent opponents, but don't let it empty the pool
  const freshCandidates = bracketCandidates.filter(
    (c) => !recentOpponentIds.has(c.userId),
  );
  const pool = (freshCandidates.length > 0 ? freshCandidates : bracketCandidates)
    .slice(0, ELO_MATCHMAKING_POOL_SIZE);

  if (pool.length === 0) return null;

  // Weighted random: closer ELO matches get higher probability
  // Weight = 1 / (1 + |eloDiff| / 50) — still mostly fair, slightly favors closer
