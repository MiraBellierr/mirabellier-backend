...
function calculateEloExchange(winner, loser) {
  ...
  const useProvisionalK =
    isEloProvisional(winner?.eloMatches) ||
    isEloProvisional(loser?.eloMatches);
  const kFactor = useProvisionalK ? ELO_PROVISIONAL_K : ELO_ESTABLISHED_K;
  const winnerExpected =
    1 / (1 + 10 ** ((loserRating - winnerRating) / ELO_SCALE));
  const requestedDelta = Math.max(
    0,
    Math.round(kFactor * (1 - winnerExpected)),
  );
  const delta = Math.min(requestedDelta, loserRating - ELO_MIN_RATING);
  ...
  const candidates = db
    .prepare(
      `SELECT p.*
       ...
       ORDER BY ABS(COALESCE(p.eloRating, ?) - ?) ASC,
                COALESCE(p.eloMatches, 0) DESC,
                p.updatedAt ASC
       LIMIT ?`,
    )
    .all(
