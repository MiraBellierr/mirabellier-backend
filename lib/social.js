...
const TRANSCODE_TIMEOUT_MS = 600000;

// Safari on iOS (and iPads) can only decode a narrow set of MP4 codecs when
// streamed over HTTP. Instagram/YouTube exports often land as HEVC (hvc1) or
// AV1, which desktop Chrome plays but iPhones cannot. These helpers probe the
// downloaded file and re-encode anything incompatible to H.264/AAC + faststart.
...
async function transcodeToH264(filePath) {
  ...
  const videoIsCompatible =
    probe.hasVideo && PLAYABLE_VIDEO_CODECS.has(probe.videoCodec);
