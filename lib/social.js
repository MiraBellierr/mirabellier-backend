...
// These helpers probe the file and, when needed, produce an H.264/AAC MP4
// with faststart while KEEPING the original resolution and frame rate, so
// the uploaded HD quality survives the conversion.
const PLAYABLE_VIDEO_CODECS = new Set(["h264"]);
const PLAYABLE_AUDIO_CODECS = new Set(["aac", "mp3"]);
...
// The uploaded/imported file may live in a container (or use codecs) that not
// every device can stream. transcodeToH264 below returns the file that should
// be stored:
//   - already 8-bit H.264/AAC in an MP4 container → kept untouched
//     (full quality)
//   - 8-bit H.264/AAC but not MP4 (e.g. .mov/.mkv) → remuxed with faststart
//   - anything else (HEVC, AV1, VP9, 10-bit, Opus…) → re-encoded to 8-bit
//     H.264/AAC at CRF 18 with the original resolution and frame rate
//     preserved; 10-bit HDR (PQ/HLG) sources get tone-mapped to SDR BT.709
...
async function transcodeToH264(filePath) {
  ...
  const bitDepth = pixelFormatBitDepth(probe.pixelFormat);
  const hasHighDepthVideo = probe.hasVideo && (bitDepth ?? 8) > 8;
  // H.264 in 10-bit (High10) or HDR is NOT what iPhones can decode — treat
  // any high-bit-depth stream as needing an 8-bit H.264 conversion.
  const videoIsCompatible =
    probe.hasVideo &&
    PLAYABLE_VIDEO_CODECS.has(probe.videoCodec) &&
    !hasHighDepthVideo;
  const audioIsCompatible =
    !probe.hasAudio || PLAYABLE_AUDIO_CODECS.has(probe.audioCodec);
  ...
  if (videoIsCompatible && audioIsCompatible) {
    ...
    return remuxToFaststartMp4(filePath, probe);
  }
  ...
  // HDR (10-bit with PQ/HLG brightness metadata, e.g. iPhone recordings):
  // tone map to SDR BT.709 instead of truncating, or everything looks flat.
  const isHdr = hasHighDepthVideo && HDR_TRANSFER_CODES.has(probe.colorTransfer);
  let usesToneMap = false;
  if (isHdr && (await ffmpegHasZscale())) {
    usesToneMap = true;
  }
  ...
  const args = [
    ...
    "-preset",
    "slow",
    // CRF 18 is visually near-lossless — no scale or -r flag is passed, so
    // the output keeps the original resolution and frame rate.
    "-crf",
    "18",
  ];
  if (usesToneMap) {
    args.push("-vf", HDR_TO_SDR_FILTER);
  }
  args.push("-pix_fmt", "yuv420p", "-profile:v", "high");
  if (isHdr) {
    // Explicitly tag the output as SDR BT.709. Without this, players that
    // see leftover HDR metadata keep treating the video as HDR even when the
    // tone map could not run (ffmpeg without zscale support).
    args.push(
      "-color_primaries",
      "bt709",
      "-color_trc",
