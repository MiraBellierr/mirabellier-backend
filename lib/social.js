...
async function fetchYtDlpInfo(url, options = {}) {
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    ...
    try {
      ...
      if (attempt === 0 && (err.code === "YTDLP_FAILED" || err.code === "YTDLP_TIMEOUT")) {
        await new Promise((resolve) => setTimeout(resolve, 1500));
        continue;
      }
      return { json: null, ownerAvatarUrl: null, error: err };
    } finally {
      if (pagesDir) {
...
async function downloadWithYtDlp(url, destBase, options = {}) {
  ...
  for (let i = 0; i < FORMAT_FALLBACKS.length; i += 1) {
    ...
    try {
      ...
      lastError = err;
      const canRetry = isFormatSelectionError(err) && i < FORMAT_FALLBACKS.length - 1;
